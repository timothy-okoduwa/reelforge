...
## Part 5: Deploy Worker to Railway

1. Sign up at https://railway.app
2. New project → Deploy from GitHub repo → Select the `/worker` folder
   - Alternatively, push `/worker` as a separate GitHub repo
...
5. After deployment, copy the Railway public URL → paste as `RAILWAY_WORKER_URL` in `.env.local`
...
- [ ] Check Railway logs — worker receives render request
...
- [ ] Stripe/Lemon Squeezy webhook test — subscription updates user plan
...
| `RAILWAY_WORKER_URL` | Railway deployment URL |
| `NEXT_PUBLIC_APP_URL` | Your Vercel production URL |
